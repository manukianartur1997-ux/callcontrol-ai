...
const copy = {
  ru: {
    ...
    compareTitle: "CallControl vs Gong, Chorus и Fireflies",
    ...
    compareCols: ["CallControl (под ключ)", "Gong / Chorus", "Fireflies / Avoma"],
    compareRows: [
      ["Что это", "Управленческий аудит как услуга", "Платформа revenue intelligence", "AI-нотетейкер и транскрипты"],
      ["Что вы получаете", "PDF-отчёт, выводы и план действий", "Дашборды и метрики звонков", "Заметки, саммари, транскрипты"],
      ["Кто делает работу", "Я разбираю звонки за вас", "Ваш РОП внедряет и анализирует", "Вы читаете заметки сами"],
      ["Внедрение", "Нет — присылаете записи", "Недели настройки и обучения", "Подключение к календарю"],
      ["Стоимость", "От $700 за разовый аудит", "$130K+ в год за команду 50 чел.", "$10-19 за пользователя в месяц"],
      ["Время до результата", "5-7 рабочих дней", "Месяцы до зрелого процесса", "Сразу, но без выводов"],
    ],
    compareNote: "Сравнение по публичной информации о категориях продуктов. CallControl не заменяет CRM или платформу — он отвечает на вопрос «где мы теряем деньги и что чинить первым».",
...
Object.assign(copy.uk, {
  ...
  compareTitle: "CallControl vs Gong, Chorus і Fireflies",
  ...
  compareCols: ["CallControl (під ключ)", "Gong / Chorus", "Fireflies / Avoma"],
  compareRows: [
    ["Що це", "Управлінський аудит як послуга", "Платформа revenue intelligence", "AI-нотетейкер і транскрипти"],
    ["Що ви отримуєте", "PDF-звіт, висновки і план дій", "Дашборди і метрики дзвінків", "Нотатки, саммарі, транскрипти"],
    ["Хто робить роботу", "Я розбираю дзвінки за вас", "Ваш керівник впроваджує й аналізує", "Ви читаєте нотатки самі"],
    ["Впровадження", "Немає — надсилаєте записи", "Тижні налаштування і навчання", "Підключення до календаря"],
    ["Вартість", "Від $700 за разовий аудит", "$130K+ на рік за команду 50 осіб", "$10-19 за користувача на місяць"],
    ["Час до результату", "5-7 робочих днів", "Місяці до зрілого процесу", "Одразу, але без висновків"],
  ],
  compareNote: "Порівняння за публічною інформацією про категорії продуктів. CallControl не замінює CRM чи платформу — він відповідає на питання «де ми втрачаємо гроші і що чинити першим».",
...
Object.assign(copy.en, {
  ...
  compareNav: "Compare",
  compareTitle: "CallControl vs Gong, Chorus, and Fireflies",
  ...
  compareCols: ["CallControl (done-for-you)", "Gong / Chorus", "Fireflies / Avoma"],
  compareRows: [
    ["What it is", "A management audit, as a service", "Revenue intelligence platform", "AI note-taker and transcripts"],
    ["What you get", "PDF report, findings, action plan", "Dashboards and call metrics", "Notes, summaries, transcripts"],
    ["Who does the work", "I review the calls for you", "Your sales lead deploys and analyzes", "You read the notes yourself"],
    ["Onboarding", "None — you send recordings", "Weeks of setup and training", "Calendar / call connection"],
    ["Cost", "From $700 per one-time audit", "$130K+/yr for a 50-rep team", "$10-19 per user / month"],
    ["Time to insight", "5-7 business days", "Months to a mature process", "Instant notes, no verdict"],
  ],
  compareNote: "Comparison based on public information about each product category. CallControl does not replace your CRM or platform — it answers “where are we losing money and what do we fix first.”",
...
// Inline SVG favicon (brand teal->indigo gradient mark) as a data URI.
// Avoids a 404 on /favicon.ico and needs no extra asset in the build copy list.
const FAVICON =
  "data:image/svg+xml," +
  encodeURIComponent(
...
function render(locale) {
  ...
  const langs = ["ru", "uk", "en"].map((l) => `<a class="lang-pill${l === locale ? " active" : ""}" href="${url(l)}" hreflang="${l}" lang="${l}" aria-label="${langNames[l]}"${l === locale ? ' aria-current="page"' : ""}>${l === "uk" ? "UA" : l.toUpperCase()}</a>`).join("");
  ...
  const compareHead = `<div class="cmp-row cmp-head"><span class="cmp-feat"></span>${t.compareCols.map((c, i) => `<span class="cmp-col${i === 0 ? " cmp-own" : ""}">${esc(c)}</span>`).join("")}</div>`;
  const compareBody = t.compareRows.map((row) => {
    const [feat, ...cells] = row;
    const cols = cells.map((c, i) => `<span class="cmp-col${i === 0 ? " cmp-own" : ""}" data-label="${esc(t.compareCols[i])}">${i === 0 ? '<i class="cmp-tick" aria-hidden="true">✓</i>' : ""}${esc(c)}</span>`).join("");
    return `<div class="cmp-row"><span class="cmp-feat">${esc(feat)}</span>${cols}</div>`;
  }).join("");
  const compareTable = `<div class="cmp-table" role="table" aria-label="${esc(t.compareTitle)}">${compareHead}${compareBody}</div>`;
  const demoHub = [
...
.button:focus-visible,.nav-cta:focus-visible,.tier-card a:focus-visible{outline-offset:3px}
...
/* comparison */
.cmp-wrap{border:1px solid var(--line);border-radius:var(--radius);background:var(--card);overflow:hidden}
.cmp-table{display:grid}
.cmp-row{display:grid;grid-template-columns:minmax(140px,1.1fr) repeat(3,minmax(0,1fr));align-items:center;gap:14px;padding:16px 20px;border-bottom:1px solid var(--line)}
.cmp-row:last-child{border-bottom:0}
.cmp-head{background:rgba(255,255,255,.03)}
.cmp-head .cmp-col{font:600 13px "JetBrains Mono",monospace;color:var(--muted);text-transform:none;letter-spacing:0}
.cmp-head .cmp-col.cmp-own{color:#bae6fd}
.cmp-feat{font-weight:600;font-size:14px;color:var(--ink)}
.cmp-head .cmp-feat{font:600 11px "JetBrains Mono",monospace;color:#6b7c93;text-transform:uppercase;letter-spacing:.05em}
.cmp-col{color:var(--muted);font-size:14px;line-height:1.45;display:flex;align-items:flex-start;gap:7px;align-self:stretch}
.cmp-col.cmp-own{color:#e0f2fe;font-weight:600;margin:-16px 0;padding:16px 14px;background:linear-gradient(180deg,rgba(56,189,248,.09),rgba(99,102,241,.05));border-left:1px solid rgba(125,211,252,.16);border-right:1px solid rgba(125,211,252,.16)}
.cmp-head .cmp-col.cmp-own{border-top:1px solid rgba(125,211,252,.2)}
.cmp-row:last-child .cmp-col.cmp-own{border-bottom:1px solid rgba(125,211,252,.2)}
.cmp-tick{flex:none;display:grid;place-items:center;width:18px;height:18px;border-radius:50%;background:linear-gradient(135deg,#38bdf8,#6366f1);color:#fff;font-size:11px;font-style:normal;font-weight:700;line-height:1;margin-top:1px}
.cmp-note{margin-top:16px;color:#6b7c93;font-size:13px;line-height:1.55;max-width:760px}
/* pricing */
...
@media(max-width:760px){.cmp-wrap{border:0;background:transparent}.cmp-table{display:grid;gap:12px}.cmp-head{display:none}.cmp-row{grid-template-columns:1fr;gap:0;padding:0;border:1px solid var(--line);border-radius:var(--radius);background:var(--card);overflow:hidden}.cmp-feat{padding:13px 16px;background:rgba(255,255,255,.03);border-bottom:1px solid var(--line);font-size:15px}.cmp-col{justify-content:space-between;padding:12px 16px;border-bottom:1px solid var(--line)}.cmp-col:last-child{border-bottom:0}.cmp-col::before{content:attr(data-label);color:#6b7c93;font:600 11px "JetBrains Mono",monospace;text-transform:none;flex:none;margin-right:14px}.cmp-col.cmp-own{margin:0;border-left:0;border-right:0;background:linear-gradient(180deg,rgba(56,189,248,.09),rgba(99,102,241,.05))}.cmp-head .cmp-col.cmp-own,.cmp-row:last-child .cmp-col.cmp-own{border-top:0;border-bottom:1px solid var(--line)}}
